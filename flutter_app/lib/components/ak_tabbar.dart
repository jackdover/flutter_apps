...
class AKTab extends StatelessWidget {
...
class AKTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  ///
  /// If [indicator] is not null or provided from [AKTabBarTheme],
  ...
  /// and the length of the [AKTabBarView.children] list.
  ...
  /// and the entire [AKTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// property is used.
  ///
  /// If [indicator] is specified or provided from [AKTabBarTheme],
  ...
  /// is [AKTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [AKTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  /// the tab widget itself.
  final Decoration indicator;

  // 自定义tab指示器
  // final Widget indicator2;//todo

  /// Defines how the selected tab indicator's size is computed.
  ///
  /// The size of the selected tab indicator is defined relative to the
  /// tab's overall bounds if [indicatorSize] is [AKTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [AKTabBarIndicatorSize.label].
  ///
  ...
  /// An optional callback that's called when the [AKTabBar] is tapped.
  ///
  /// The callback is applied to the index of the tab where the tap occurred.
  ///
  /// This callback has no effect on the default handling of taps. It's for
  /// applications that want to do a little extra work when a tab is tapped,
  /// even if the tap doesn't change the TabController's index. AKTabBar [onTap]
  /// callbacks should not make changes to the TabController since that would
  /// interfere with the default tap handler.
  final ValueChanged<int> onTap;

  /// How the [AKTabBar]'s scroll view should respond to user input.
  ///
  ...
  Size get preferredSize {
    for (final Widget item in tabs) {
      if (item is AKTab) {
        final AKTab tab = item;
        ///做一下判断，保证最小高度
        if (tab.tabHeight > _kTabHeight) {
          return Size.fromHeight(tab.tabHeight /*+ indicatorWeight*/);
        }
      }
    }
  ...
  @override
  _AKTabBarState createState() => _AKTabBarState();
}
...
class _AKTabBarState extends State<AKTabBar> {
  ...
  Decoration get _indicator {
    ...
    final TabBarTheme AKTabBarTheme = TabBarTheme.of(context);
    if (AKTabBarTheme.indicator != null) return AKTabBarTheme.indicator;
  ...
  // If the AKTabBar is rebuilt with a new tab controller, the caller should
  ...
  @override
  void didUpdateWidget(AKTabBar oldWidget) {
  ...
  void _handleTabControllerAnimationTick() {
    ...
    if (!_controller.indexIsChanging && widget.isScrollable) {
      // Sync the AKTabBar's scroll position with the AKTabBarView's PageView.
      _currentIndex = _controller.index;
  ...
  Widget build(BuildContext context) {
    ...
    assert(() {
      if (_controller.length != widget.tabs.length) {
        throw FlutterError("Controller's length property (${_controller.length}) does not match the "
            "number of tabs (${widget.tabs.length}) present in AKTabBar's tabs property.");
      }
      return true;
    }());
    ...
    for (int i = 0; i < widget.tabs.length; i += 1) {
      wrappedTabs[i] = Center(
        heightFactor: 1.0,
        child: Padding(
          padding: widget.labelPadding ?? AKTabBarTheme.labelPadding ?? kTabLabelPadding,
          child: KeyedSubtree(
            key: _tabKeys[i],
            child: widget.tabs[i],
          ),
    ...
    if (_controller != null) {
      ...
      if (_controller.indexIsChanging) {
        ...
        // The user is dragging the AKTabBarView's PageView left or right.
    ...
    if (widget.isScrollable) {
      _scrollController ??= _AKTabBarScrollController(this);
      AKTabBar = SingleChildScrollView(
...
class _AKTabBarScrollController extends ScrollController {
  _AKTabBarScrollController(this.tabBar);

  final _AKTabBarState tabBar;

  @override
  ScrollPosition createScrollPosition(ScrollPhysics physics, ScrollContext context, ScrollPosition oldPosition) {
    return _AKTabBarScrollPosition(
      physics: physics,
      context: context,
...
class _AKTabBarScrollPosition extends ScrollPositionWithSingleContext {
  _AKTabBarScrollPosition({
